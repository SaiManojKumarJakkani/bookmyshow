...
public class ShowDetailsDAO {
    ...
    public List<List<Object>> getShowDetailsOnDate(List<Integer> listOfVenueIDs, LocalDate date){
        List<List<Object>> showList = new ArrayList<>();
        // Create the SQL query string with dynamic placeholders
        String sql = "SELECT Shows.*, Venue.name AS venueName, Hall.* FROM "+tableName+" JOIN Venue ON Shows.venueID = Venue.venueID JOIN Hall ON Shows.hallID = Hall.hallID WHERE showDate = '"+date+"' AND Shows.venueID IN (";
        for (int i = 0; i < listOfVenueIDs.size(); i++) {
            sql += "?"; // Add a placeholder
            if (i < listOfVenueIDs.size() - 1) {
                sql += ", "; // Add a comma if not the last placeholder
    ...
    public List<List<Object>> getShowDetailsMovie(List<Integer> listOfVenueIDs,LocalDate startDate,LocalDate endDate, String movieName){
        ...
        String sql = "SELECT Shows.*, Venue.name AS venueName, Hall.*  FROM "+tableName+" JOIN Venue ON Shows.venueID = Venue.venueID JOIN Hall ON Shows.hallID = Hall.hallID WHERE Shows.showDate  BETWEEN '"+startDate+"' AND '"+endDate+"'  AND Shows.venueID IN (";
        for (int i = 0; i < listOfVenueIDs.size(); i++) {
            sql += "?"; // Add a placeholder
            if (i < listOfVenueIDs.size() - 1) {
                sql += ", "; // Add a comma if not the last placeholder
        ...
        sql += ") AND Shows.movieName = '"+movieName+"' ORDER BY venueName ASC;";
        try{
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
